...
import { CreditCard, Zap, TrendingUp, Package, Crown, Sparkles, Check, ExternalLink, AlertCircle } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto px-6 py-10">
      {showSuccess && (
        <div className="mb-6 p-4 rounded-xl flex items-center gap-3"
          ...
          <span style={{ fontSize: "14px", color: "#16a34a" }}>
            {searchParams.get("upgraded") ? "Abonnement activé — crédits crédités !" : "Crédits ajoutés à ton compte !"}
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="col-span-1 rounded-xl p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: "11px", fontWeight: 500, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
            Solde
          </div>
          <div style={{ fontSize: "36px", fontWeight: 600, color: "var(--foreground)", lineHeight: 1.2, marginTop: 6 }}>
            {profile?.credits ?? 0}
          </div>
        ...
        <div className="col-span-1 rounded-xl p-5 flex flex-col justify-between"
          style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          <div style={{ fontSize: "11px", fontWeight: 500, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
            Rollover
          </div>
          <div style={{ fontSize: "14px", color: "var(--foreground)", marginTop: 6, lineHeight: 1.4 }}>
            Tes crédits n'expirent jamais
          </div>
          {profile?.stripe_subscription_id && (
            <button onClick={handlePortal} disabled={checkoutLoading === "portal"}
      ...
      {/* Upgrade plans (only if not studio) */}
      {profile?.plan !== "studio" && (
        <div className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            {PLANS.filter(p => {
              if (profile?.plan === "generate") return p.key === "studio";
              return true;
            }).map(plan => (
              <div key={plan.key} className="rounded-xl p-6 flex flex-col"
                ...
                {plan.highlighted && (
                  <span className="inline-block mb-3 px-2 py-0.5 rounded-full text-white"
                    style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.05em", background: "var(--ora-signal)", width: "fit-content" }}>
                    RECOMMANDÉ
                  </span>
                )}
                <div className="flex items-end gap-1 mb-4">
                  <span style={{ fontSize: "32px", fontWeight: 600, letterSpacing: "-0.02em", color: "var(--foreground)" }}>
                    €{plan.price}
                  </span>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: plan.highlighted ? "var(--ora-signal)" : "var(--secondary)",
                  ...
                  {checkoutLoading === plan.key ? "Redirection…" : `Activer ${plan.name}`}
                </button>
              </div>
            ))}
          </div>
      ...
      {/* Credit packs */}
      <div className="mb-10">
        <h2 style={{ fontSize: "16px", fontWeight: 500, color: "var(--foreground)", marginBottom: 4 }}>
          Acheter des crédits
        </h2>
        <p style={{ fontSize: "13px", color: "var(--muted-foreground)", marginBottom: 16 }}>
          Sans engagement. Les crédits n'expirent jamais et s'ajoutent à ton solde.
        </p>
        <div className="grid grid-cols-3 gap-4">
          {PACKS.map(pack => (
            <div key={pack.key} className="rounded-xl p-5 flex flex-col"
              ...
              <div style={{ fontSize: "16px", fontWeight: 500, color: "var(--foreground)", marginTop: 6 }}>
                {pack.label}
              </div>
              <div style={{ fontSize: "12px", color: "var(--muted-foreground)", marginTop: 2, marginBottom: 16 }}>
                {pack.per} · rollover illimité
              </div>
              <button
                onClick={() => handleCheckout("pack", pack.key)}
                disabled={checkoutLoading === pack.key}
                className="w-full py-2 rounded-lg transition-all mt-auto"
                style={{
                  fontSize: "13px", fontWeight: 500,
                  background: "var(--secondary)",
                  color: "var(--foreground)",
                  border: "1px solid var(--border)",
                  cursor: checkoutLoading === pack.key ? "wait" : "pointer",
                  opacity: checkoutLoading === pack.key ? 0.7 : 1,
                }}>
                {checkoutLoading === pack.key ? "Redirection…" : "Acheter"}
              </button>
            </div>
          ))}
        </div>
      ...
      {/* Credit cost table */}
      <div className="mb-10">
        <h2 style={{ fontSize: "16px", fontWeight: 500, color: "var(--foreground)", marginBottom: 12 }}>
          Coût par action
        </h2>
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
          {COSTS.map((item, i) => (
            <div key={item.action} className="flex items-center justify-between px-4 py-3"
              style={{
                background: "var(--card)",
                borderBottom: i < COSTS.length - 1 ? "1px solid var(--border)" : "none",
              }}>
              <span style={{ fontSize: "13px", color: "var(--foreground)" }}>{item.action}</span>
              <span style={{ fontSize: "13px", fontWeight: 500, color: "var(--ora-signal)" }}>
                {item.credits} cr
              </span>
            </div>
          ))}
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            {transactions.slice(0, 20).map((tx, i) => (
              <div key={tx.id} className="flex items-center justify-between px-4 py-3"
                style={{
                  background: "var(--card)",
                  borderBottom: i < Math.min(transactions.length, 20) - 1 ? "1px solid var(--border)" : "none",
                }}>
                <div>
                ...
                <span style={{
                  fontSize: "13px", fontWeight: 500,
                  color: tx.amount > 0 ? "#22c55e" : "var(--muted-foreground)",
                }}>
