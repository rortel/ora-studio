  ...
  return (
    <div className={`relative w-full max-w-[560px] h-[280px] ${className}`}>
      ...
      <div className="absolute left-[4%] top-1/2 -translate-y-1/2 flex items-center gap-2">
        <div className="w-3 h-3 rounded-full bg-[var(--ora-signal)]" />
        <span className="text-xs tracking-wide text-[var(--muted-foreground)] uppercase">Source</span>
      </div>
      ...
      {targets.map((t, i) => (
        ...
        >
          <div className="w-1.5 h-1.5 rounded-full" style={{ background: "rgba(59,79,196,0.5)" }} />
          <span className="text-xs tracking-wide" style={{ color: "rgba(17,17,19,0.7)" }}>{t.label}</span>
        </motion.div>
      ))}
    </div>
  );
